    ...
    <div className="signup-popup">
      ...
      <form onSubmit={props.submitHandler}>
        ...
        <div className="panel-buttons">
          <input type="submit" className={props.formIsValid ? "signup-button" : "signup-button-false"} value="회원가입하기" disabled={!props.formIsValid} />
        </div>
      </form>
    </div>
  );
...
const phoneNumReducer = (state, action) => {
  const phoneNumCheck = /\d{11}/;

  if (action.type === "PHONE_NUM_INPUT") {
    return { value: action.val, isValid: phoneNumCheck.test(action.val), falseEffect: phoneNumCheck.test(action.val) };
  }
  return { value: "", isValid: false, falseEffect: true };
};
...
const passwordReducer = (state, action) => {
  ...
  if (action.type === "PASSWORD_INPUT") {
    return { value: action.val, isValid: passwordCheck.test(action.val), falseEffect: passwordCheck.test(action.val), compareValue: state.compareValue, compareIsValid: state.compareIsValid, compareFalseEffect: state.compareFalseEffect };
  }
  if (action.type === "PASSWORD_COMPARE") {
    return { value: state.value, isValid: state.isValid, falseEffect: state.falseEffect, compareValue: action.val, compareIsValid: state.value === action.val, compareFalseEffect: state.value === action.val };
  }
...
const Signup = (props) => {
  ...
  const [passwordState, dispatchPassword] = useReducer(passwordReducer, { value: "", isValid: false, falseEffect: true, compareValue: "", compareIsValid: true, compareFalseEffect: true });
  ...
  const checkPasswordChangeHandler = (event) => {
    dispatchPassword({ type: "PASSWORD_COMPARE", val: event.target.value });
  };
  ...
  useEffect(() => {
    if (nameIsValid && phoneNumIsValid && passwordIsValid && passwordState.compareIsValid && allChecked) {
      setFormIsValid(true);
    } else {
      setFormIsValid(false);
    }
  }, [nameIsValid, phoneNumIsValid, passwordIsValid, passwordState.compareIsValid, allChecked]);
    ...
    <div>
      ...
      {ReactDOM.createPortal(
        <ModalOverlay
          ...
          phoneNumChangeHandler={phoneNumChangeHandler}
          phoneNumIsValid={phoneNumIsValid}
          phoneNumFalseEffect={phoneNumFalseEffect}
          passwordChangeHandler={passwordChangeHandler}
          passwordIsValid={passwordIsValid}
          passwordFalseEffect={passwordFalseEffect}
          checkPasswordChangeHandler={checkPasswordChangeHandler}
          compareIsValid={passwordState.compareIsValid}
          compareFalseEffect={passwordState.compareFalseEffect}
